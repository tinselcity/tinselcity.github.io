...
#### Popularity
Real internet traffic is not so random.  Some URL's are much more popular than others, and the distribution of usage might have something like a [zipf](https://en.wikipedia.org/wiki/Zipf%27s_law) distribution.  Accounting for this might mean load for popular URL's have to be spread to other servers as well.  An approach to spreading load for popular URL's, could be to rotate between the top N highest random weighted servers, as opposed to just the first.  This, adds complexity, however, and state must be shared across CARP'ing servers in order to preserve consensus, about which servers popular URL's would be proxied to.
